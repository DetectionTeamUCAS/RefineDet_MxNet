...
class VGGAtrousExtractor(VGGAtrousBase):
    ...
    def hybrid_forward(self, F, x, init_scale):
        ...
        for extra in self.extras:
            x = extra(x)
            outputs["ARM_features"].append(x)  # conv6_2
        ...
        for i in range(3, 0, -1):  # P5, P4, P3
            deep_fet, shallow_fet = outputs["ODM_features"][i], outputs["ARM_features"][i-1]
            upsampled_fet = self.upsamples[i-1](deep_fet)
            ...
            outputs["ODM_features"][i-1] = self.fusions[i-1](ele_sum)
...
extra_spec = {
    320: [((256, 1, 1, 0), (512, 3, 2, 1))],  # only two extra conv layers
